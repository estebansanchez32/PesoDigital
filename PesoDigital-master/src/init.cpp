...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/PesoDigital.conf are parsed in qt/bitcoin.cpp's main()
        ...
        {
            // First part of help message is specified in PesoDigitald / RPC client
            std::string strUsage = _("PesoDigital version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  PesoDigitald [options]                     " + "\n" +
                  "  PesoDigitald [options] <command> [params]  " + _("Send command to server or PesoDigitald") + "\n" +
                  "  PesoDigitald [options] help                " + _("List commands") + "\n" +
                  "  PesoDigitald [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "PesoDigital:"))
                fCommandLine = true;
...
{
    ThreadSafeMessageBox(str, _("PesoDigital"), wxOK | wxMODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    ThreadSafeMessageBox(str, _("PesoDigital"), wxOK | wxICON_EXCLAMATION | wxMODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -conf=<file>          "   + _("Specify configuration file (default: PesoDigital.conf)") + "\n" +
        "  -pid=<file>           "   + _("Specify pid file (default: PesoDigitald.pid)") + "\n" +
    ...
    strUsage += string() +
        _("\nSSL options: (see the PesoDigital Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-reservebalance")) // PesoDigital: reserve balance amount
    {
    ...
    if (mapArgs.count("-checkpointkey")) // PesoDigital: checkpoint master priv key
    {
    ...
    // Make sure only a single PesoDigital process is using the data directory.
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  PesoDigital is probably already running."), pszDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("PesoDigital version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    //
    if (fDaemon)
        fprintf(stdout, "PesoDigital server starting\n");
    ...
    if (mapArgs.count("-onlynet")) {
        ...
        BOOST_FOREACH(std::string snet, mapMultiArgs["-onlynet"]) {
            ...
            if (net == NET_UNROUTABLE) {
                ThreadSafeMessageBox(_("Unknown network specified in -onlynet"), _("PesoDigital"), wxOK | wxMODAL);
                return false;
            }
            nets.insert(net);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        if (nLoadWalletRet == DB_CORRUPT)
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of PesoDigital") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart PesoDigital to complete") << "\n";
