...
// PesoDigital: optional setting to unlock wallet for block minting only;
...
// PesoDigital: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // PesoDigital: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            for (;;)
            {
                ...
                // PesoDigital: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    if (!GetBoolArg("-avatar")) // PesoDigital: not avatar mode
                    {
...
// PesoDigital: create coin stake transaction
...
// PesoDigital: check 'spent' consistency between wallet and txindex
// PesoDigital: fix wallet spent state according to txindex
...
// PesoDigital: disable transaction (only for coinstake)
